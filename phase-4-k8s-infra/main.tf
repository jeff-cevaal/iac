...
resource "azurerm_key_vault" "iac_vault" {
  name                = "kv-n8n-iac"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
...
resource "azurerm_role_assignment" "kv_admin" {
  scope                = azurerm_key_vault.iac_vault.id
...
resource "azurerm_key_vault_secret" "db_host" {
  name         = "db-host"
  value        = azurerm_postgresql_flexible_server.n8n_db.fqdn
  key_vault_id = azurerm_key_vault.iac_vault.id
...
output "key_vault_name" {
  value = azurerm_key_vault.iac_vault.name
}

output "key_vault_uri" {
  value = azurerm_key_vault.iac_vault.vault_uri
}
